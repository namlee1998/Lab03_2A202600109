...
TravelWise là một travel assistant agent được xây dựng theo kiến trúc ReAct (Reason + Act), sử dụng các công cụ thực tế (Brave Search, Calculator, System Time) để lên kế hoạch du lịch cho người dùng.

Trong quá trình phát triển Agent v1, chúng tôi phát hiện và phân tích **4 lỗi nghiêm trọng** thông qua telemetry logs, sau đó xây dựng Agent v2 với các bản vá tương ứng.

- **Tổng sessions thử nghiệm**: 21
- **Sessions hoàn thành đúng (có bước thực)**: 13/18 (72%)
- **Sessions bị hallucinate loop (Bug #1, trước khi fix)**: 5 sessions (steps = 0)
...
| :---------------- | :--------------------------------------------------- | :---------------------------------------------------------------------------------------- |
| `web_search`      | `string` — plain text query                          | Tìm thời tiết, giá vé, khách sạn, địa điểm ăn uống theo thời gian thực (Brave Search API) |
...
- **Fix**: Thêm rule vào system prompt cấm dùng quotes; thêm `.replace('"', '')` khi parse tool_args.
...
| Tính tổng chi phí                  | ⚠️ Tính trực tiếp, có thể sai            | ✅ Dùng calculator            | **Agent**                  |
...
**Kết luận:** ReAct Agent vượt trội với câu hỏi cần real-time data hoặc multi-step reasoning. Chatbot baseline nhanh hơn và tốn ít token hơn với câu hỏi đơn giản.
...
> **Submission**: Đổi tên file thành `GROUP_REPORT_[TÊN_TEAM].md` và đặt trong thư mục `report/group_report/`.
